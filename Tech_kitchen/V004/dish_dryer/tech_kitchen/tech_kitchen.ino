...
//define the path 0: Both Bluetooth and Serial port 1: Serial port only  
int Path = 1;
int Debug = 1; // 0: debug disable, 1:debug enable. 
...
int RH_allswitch_pin = 7;
int RH_fanswitch_pin = 8;  
int pin09 = 9; //PMW
int pin10 = 10; //PMW
int pin11 = 11; //PMW
int RH_lightswitch_pin = 12; 
int pin13 = 13; 
...
{ 
  if (Path == 0) 
  { 
      char c = meetAndroid.receive(); // you need to keep this in your loop() to receive events
      if (Debug == 1)
      {
  ...
  if (Serial.available()>0){
      ...
      Serial.print("Arduino get the char: ");
...
void fading(byte flag, byte numOfValues) 
{
  int fadingdelay = 10;
// fade in from min to max in increments of 5 points:
  for(int fadeValue = 0 ; fadeValue <= 255; fadeValue +=3) { 
    // sets the value (range from 0 to 255):
    analogWrite(pin09, fadeValue);         
